...
public:
    ...
    Client(const int id, const std::string& name, const std::string& email, const std::string& phone, const std::string& company, const std::string& notes) : 
        SQLiteORM("clients"), id(id), name(name), email(email), phone(phone), company(company), notes(notes) {
        registerFields();
    }
